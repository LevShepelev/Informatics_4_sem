...
int TCP_communication(int socket_tcp) {
    ...
    unsigned key = Get_symm_key(client_fd, &client, &client_len);
    log_info("Symm_key = %u\n", key);
    if (client_fd < 0) {
        log_perror("accept");
        return 1;
...
int UDP_communication(int socket_udp, struct sockaddr_in* client) {
    ...
    unsigned key = Get_symm_key(socket_udp, client, &client_len);
...
int Authetification(int fd_in, int fd_out, int fdm, int is_udp, int key) {
    ...
    else printf_fd_safe(fd_out, key, "login: ");
    while (ret <= 0) {
    ...
    if (!info) {
        ...
        else printf_fd_safe(fd_out, key, "Wrong name of user, try again\n");
        log_perror("getpwnam");
        return -1;
    }
    if (is_udp)
        printf_fd(fd_out, "password: ");
    else printf_fd_safe(fd_out, key, "password: ");
    if (Login_into_user(buf)) {
        if (is_udp)
            printf_fd(fd_out, "Unseccesfull authentification\n");
        else printf_fd_safe(fd_out, key, "Unseccesfull authentification\n");
        return -1;
    }
    ...
    write(fdm, dir_name, strlen(dir_name));
    return 0;
}
...
int UDP_terminal_transmitting(struct pollfd fd_in[3], int fdm, int fd[2], int fd_out[2], char* input, int socket_udp, struct sockaddr_in* client, socklen_t client_len, int fd_for_dir_name, int key) {
    ...
    while (1) {
        if (auth) {
            auth = Authetification(fd[0], fd_out[1], fd_for_dir_name, 1, key);
            if (auth < 0)
                continue;
        }
...
int TCP_terminal_transmitting(int client_fd, int fdm, int fd_for_dir_name, int key) {
    ...
    while (1) {
        if (auth) {
            auth = Authetification(client_fd, client_fd, fd_for_dir_name, 0, key);
...
unsigned Get_symm_key(int socket, struct sockaddr_in* client, socklen_t* client_len) {
    ...
    unsigned b = rand() % 10;
    unsigned A = 0, B = ((unsigned) powl(g, b)) % p;
    ...
    mess_size = Encrypt((char*) &B, sizeof(unsigned), &message, client_pub_key);
    if (mess_size == -1) {
